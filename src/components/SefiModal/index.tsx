  ...
  async function getAPYsInfo(): Promise<RewardsToken[]> {
    ...
    const mappedRewards: RewardsToken[] = rewards.allData
      .filter(rewards =>
        lp_tokens.find(
          element =>
            element.dst_address === rewards.inc_token.address &&
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          {(status === SefiModalState.CONFIRMATION || status === SefiModalState.CONFIRMATION_CASHBACK) && (
            <span>Claimed SEFI</span>
          )}
          <span
      ...
      >
        ...
        {status === SefiModalState.CLAIM_CASH_BACK && (
          <ClaimCashback data={data} burnCashback={burnCashback} onKeplrIcon={loginKeplr} claimInfo={claimInfo} />
        )}
